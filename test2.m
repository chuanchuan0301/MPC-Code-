...
kappa = 1.2;
params.kappa = kappa;
fprintf('System Configuration: coupled Van der Pol (Eq. 30)\n');
for i = 1:params.M
    ...
    fprintf('  Sub %d: mu=%.2f, K=[%.4f, %.4f], |lambda|_max=%.4f\n', ...
        i, mu_vdp(i), params.K{i}(1), params.K{i}(2), max(abs(eig(params.A_cl{i}))));
...
%% ==================== Run simulations (same disturbance, distributed prediction) ====================
fprintf('Running EVENT-TRIGGERED distributed MPC...\n');
[x_et, u_et, trigger_et, ~, mpc_et, fb_et, reason_et, T_omega_et] = ...
    simulate_cloud_dmpc(x0, T_sim, params, disturbances, true, false);
fprintf('  ET triggers: [%d, %d, %d]\n', numel(trigger_et{1}), numel(trigger_et{2}), numel(trigger_et{3}));
print_trigger_reasons(reason_et, params.M);

fprintf('\nRunning TIME-TRIGGERED distributed MPC...\n');
[x_tt, u_tt, trigger_tt, ~, mpc_tt, fb_tt, ~, T_omega_tt] = ...
    simulate_cloud_dmpc(x0, T_sim, params, disturbances, true, true);
fprintf('  TT triggers: [%d, %d, %d]\n\n', numel(trigger_tt{1}), numel(trigger_tt{2}), numel(trigger_tt{3}));
...
zoom1_center = [85, 0]; zoom1_mag = 3; zoom1_inset = [0.52, 0.36, 0.2, 0.15];
zoom2_center = [85, 0]; zoom2_mag = 3; zoom2_inset = [0.52, 0.36, 0.2, 0.15];

fprintf('Generating Figure 1: State x1...\n');
...
[zx1, zy1] = calc_zoom_range(ax1_main, zoom1_center, zoom1_mag, T_sim);
...
for i = 1:params.M
    [tx, y_et] = smooth_series(steps, x_et{i}(1, :));
    [~, y_tt] = smooth_series(steps, x_tt{i}(1, :));
...
fprintf('Generating Figure 2: State x2...\n');
...
[zx2, zy2] = calc_zoom_range(ax2_main, zoom2_center, zoom2_mag, T_sim);
...
for i = 1:params.M
    [tx, y_et] = smooth_series(steps, x_et{i}(2, :));
    [~, y_tt] = smooth_series(steps, x_tt{i}(2, :));
...
fprintf('Generating Figure 3: Control Input u...\n');
...
for i = 1:params.M
    plot(ax3_main, steps_u, u_et{i}(:).', '-', 'Color', et_colors{i}, ...
        'LineWidth', lw_et, 'DisplayName', sprintf('Subsystem %d ET', i));
end
for i = 1:params.M
    plot(ax3_main, steps_u, u_tt{i}(:).', '-', 'Color', tt_colors{i}, ...
        'LineWidth', lw_tt, 'DisplayName', sprintf('Subsystem %d TT', i));
end
...
fprintf('Generating Figure 4: Trigger instants...\n');
...
function [u_opt, x_opt] = solve_mpc(x0, i, Ni, params, xa, x_bar, is_distributed, u_prev)
    ...
    if nargin >= 8 && ~isempty(u_prev) && numel(u_prev) == m * Ni
        ...
        u0 = [up(:, 2:end), saturate_u(params.K{i} * x0, params, i)];
        u0 = u0(:);
    end
...
function print_trigger_reasons(trigger_reason, M)
    for i = 1:M
        fprintf('  Subsystem %d: Init=%d, Instant(16)=%d, Horizon=%d, Accum(17)=%d\n', i, ...
            sum(trigger_reason{i} == 1), sum(trigger_reason{i} == 2), ...
            sum(trigger_reason{i} == 3), sum(trigger_reason{i} == 4));
    end
end

function [xrange, yrange] = calc_zoom_range(ax, center, mag, x_total)
    y_lim = ylim(ax);
    x_half = x_total / mag / 2;
    y_half = diff(y_lim) / mag / 2;
    xrange = center(1) + [-x_half, x_half];
    yrange = center(2) + [-y_half, y_half];
    xrange(1) = max(xrange(1), 0);
    xrange(2) = min(xrange(2), x_total);
    yrange(1) = max(yrange(1), y_lim(1));
    yrange(2) = min(yrange(2), y_lim(2));
end
